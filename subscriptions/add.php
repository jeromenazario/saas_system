...
<title>Add Subscription – SaaS Manager</title>
...
<div class="container py-4" style="max-width:560px">
  <div class="card">
    <div class="card-body p-4">
      <h5 class="fw-bold mb-1"><i class="bi bi-plus-circle me-2 text-success"></i>Add Subscription</h5>
      <p class="text-muted small mb-4">For: <strong><?= htmlspecialchars($client['client_name']) ?></strong></p>
      <?php if ($error): ?><div class="alert alert-danger"><?= htmlspecialchars($error) ?></div><?php endif; ?>
      <form method="POST">
        <div class="mb-3">
          <label class="form-label fw-semibold">Plan Name <span class="text-danger">*</span></label>
          <input type="text" name="plan_name" class="form-control" placeholder="e.g. Pro, Starter, Enterprise" value="<?= htmlspecialchars($_POST['plan_name'] ?? '') ?>" required>
        </div>
        <div class="mb-3">
